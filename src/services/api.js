...
export async function getSystemHealth() {
  try {
    ...
    if (!response.ok || data.error) {
      throw new Error(
        data.error?.message || `Error mendaptkan data system health`
      );
    }
    return data;
  } catch (error) {
...
export async function getGas(network_key, to, tokenAddress = null, amount) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/estimate-gas?network_key=${network_key}&to=${to}?from=0xAb5801a7D398351b8bE11C439e05C5B3259aeC9B&tokenAddress=${tokenAddress}`
    );
    const data = await response.json();
    if (!response.ok || data.error) {
