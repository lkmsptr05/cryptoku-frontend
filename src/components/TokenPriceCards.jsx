...
function TokenPriceCards() {
  ...
  const [error, setError] = useState(null); // Logika fetch harga (tidak berubah)
  useEffect(() => {
  ...
  const getChangeColor = (change) => {
    ...
    if (parsedChange > 0)
      return "text-green-400 bg-green-900/50 px-2 py-0.5 rounded-sm";
    if (parsedChange < 0)
      return "text-red-400 bg-red-900/50 px-2 py-0.5 rounded-sm";
